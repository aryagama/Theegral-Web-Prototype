...
function initAccessibilityPanel() {
    ...
    function setBubbleOpen(opened) {
        bubbleOpen = opened;
        if (visibilityBubble) {
            visibilityBubble.classList.toggle('open', opened);
        }
    }
    ...
    if (visibilityBubble) {
        visibilityBubble.addEventListener('click', function(e) {
            if (e.target.classList.contains('bubble-hit')) return;
    ...
    document.addEventListener('click', function(e) {
        if (accessibilityPanel.classList.contains('show') &&
            ...
            !visibilityBtn.contains(e.target)) {
            hidePanel();
        }
    });
...
function initSimpleVisibilityBubble() {
    ...
    const bubbleSettings = {
        theme: localStorage.getItem('homeTheme') || 'light',
        fontSize: localStorage.getItem('homeBubbleFontSize') || 'medium',
    ...
    function renderBubbleSettings() {
        ...
        document.body.classList.add(`font-${bubbleSettings.fontSize}-simple`);
        ...
        fontState.classList.add(`size-${bubbleSettings.fontSize}`);
        if (bubbleSettings.fontSize !== 'medium') {
    ...
    colorBtn.addEventListener('click', function(event) {
        ...
        bubbleSettings.theme = bubbleSettings.theme === 'dark' ? 'light' : 'dark';
        localStorage.setItem('homeTheme', bubbleSettings.theme);
        renderBubbleSettings();
    });

    fontBtn.addEventListener('click', function(event) {
        event.stopPropagation();
        const currentIndex = fontOrder.indexOf(bubbleSettings.fontSize);
        const nextIndex = (currentIndex + 1) % fontOrder.length;
        bubbleSettings.fontSize = fontOrder[nextIndex];
        localStorage.setItem('homeBubbleFontSize', bubbleSettings.fontSize);
        renderBubbleSettings();
    });
